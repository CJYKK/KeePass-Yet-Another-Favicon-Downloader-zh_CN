...
namespace YetAnotherFaviconDownloader
{
    public sealed class FaviconDialog
    {
        ...
        public FaviconDialog(IPluginHost host)
        {
            ...
            Form fStatusDialog;
            logger = StatusUtil.CreateStatusDialog(pluginHost.MainWindow, out fStatusDialog, "Yet Another Favicon Downloader", "下载图标中……", true, false);
        ...
        private void BgWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            // Progress 100%
            ReportProgress(progress);
            status = string.Format("YAFD：成功：{0} / 未找到：{1} / 错误：{2}.", progress.Success, progress.NotFound, progress.Error);
        ...
        private void ReportProgress(ProgressInfo progress)
        {
            ...
            logger.SetText(string.Format("YAFD：成功：{0} / 未找到：{1} / 错误：{2} / 剩余：{3}", progress.Success, progress.NotFound, progress.Error, progress.Remaining), LogStatusType.Info);
        }
